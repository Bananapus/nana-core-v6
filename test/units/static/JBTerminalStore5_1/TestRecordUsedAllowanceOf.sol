...
import {JBTerminalStoreSetup5_1} from "./JBTerminalStoreSetup.sol";

contract TestRecordUsedAllowanceOf5_1_Local is JBTerminalStoreSetup5_1 {
    ...
    modifier whenAmountIsWithinRangeToUseSurplusAllowance() {
        ...
        bytes memory _currentOfCall = abi.encodeCall(IJBController.currentRulesetOf, (_projectId));
        bytes memory _currentOfReturn = abi.encode(_returnedRuleset, generateEmptyMetadata());
        mockExpect(address(_controller), _currentOfCall, _currentOfReturn);
    ...
    function test_GivenCallingCurrencyDneqAccountingCurrency() external {
        ...
        mockExpect(address(_controller), abi.encodeCall(IJBController.currentRulesetOf, (_projectId)), abi.encode(_returnedRuleset, generateEmptyMetadata()));
