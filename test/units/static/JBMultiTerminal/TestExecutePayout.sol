...
contract TestExecutePayout_Local is JBMultiTerminalSetup {
    ...
    function test_GivenTheSplitHookEQFeeless() external whenASplitHookIsConfigured {
        ...
        mockExpect(address(feelessAddresses), feelessCalldata(_hook, _noProject, address(_terminal)), abi.encode(true));
    ...
    function test_GivenTheSplitHookDNEQFeeless() external whenASplitHookIsConfigured {
        ...
        // mock call to FeelessAddresses isFeelessFor
        mockExpect(address(feelessAddresses), feelessCalldata(_hook, _noProject, address(_terminal)), abi.encode(false));
    ...
    function test_GivenThePayoutTokenIsErc20() external whenASplitHookIsConfigured {
        ...
        // mock call to FeelessAddresses isFeelessFor
        mockExpect(
            address(feelessAddresses), feelessCalldata(address(this), _noProject, address(_terminal)), abi.encode(false)
        );
    ...
    function test_GivenPreferAddToBalanceDNEQTrueAndTerminalEQThisAddress() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                JBMultiTerminal.JBMultiTerminal_MintNotAllowed.selector, _projectId, address(_terminal)
            )
        );
        JBMultiTerminal(address(_terminal))
            .executePayout({
    ...
    function test_GivenBeneficiaryEQFeeless() external {
        ...
        // mock call to FeelessAddresses isFeeless
        mockExpect(
            address(feelessAddresses), feelessCalldata(address(_bene), _projectId, address(_terminal)), abi.encode(true)
        );
    ...
    function test_WhenThereIsNoBeneficiarySplitHookOrProjectToPay() external {
        ...
        // mock call to FeelessAddresses isFeeless
        mockExpect(
            address(feelessAddresses), feelessCalldata(address(this), _projectId, address(_terminal)), abi.encode(false)
        );
