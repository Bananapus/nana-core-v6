...
contract TestMultipleAccessLimits_Local is TestBaseWorkflow {
    ...
    function testAccessConstraintsDelineation() external {
        ...
        assertEq(
            ...
            initTerminalBalance - _payoutLimits[0].amount
                + mulDiv(_payoutLimits[0].amount, JBConstants.STANDARD_FEE, JBConstants.MAX_FEE)
        );
    ...
    function testMultipleDistroLimitCurrencies() external {
        ...
        assertEq(
            ...
            ownerBalanceBeforeFirst + _amountPaidOut
                - mulDiv(_amountPaidOut, JBConstants.STANDARD_FEE, JBConstants.MAX_FEE)
        );

        // Funds leaving the ecosystem -> fee taken.
        assertEq(
            address(__terminal).balance,
            initTerminalBalance - _amountPaidOut + mulDiv(_amountPaidOut, JBConstants.STANDARD_FEE, JBConstants.MAX_FEE)
        );
        ...
        assertEq(
            _projectOwner.balance,
            _ownerBalanceBeforeNativeDist + 1 ether - mulDiv(1 ether, JBConstants.STANDARD_FEE, JBConstants.MAX_FEE)
        );

        assertEq(
            address(__terminal).balance,
            _balanceBeforeNativeDist - 1 ether + mulDiv(1 ether, JBConstants.STANDARD_FEE, JBConstants.MAX_FEE)
        );
    }
}
