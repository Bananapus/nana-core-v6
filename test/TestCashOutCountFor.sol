...
contract CashOutCountForWrapper {
    function minCashOutCountFor(
        ...
        returns (uint256)
    {
        return JBCashOuts.minCashOutCountFor(surplus, desiredOutput, totalSupply, taxTotalSupply, cashOutTaxRate);
    }
}
...
contract TestCashOutCountFor is Test {
    ...
    function test_zeroDesiredOutput() public pure {
        assertEq(JBCashOuts.minCashOutCountFor(100 ether, 0, 1000e18, 1000e18, 5000), 0);
    }

    function test_desiredOutputEqualsSurplus() public pure {
        uint256 count = JBCashOuts.minCashOutCountFor(100 ether, 100 ether, 1000e18, 1000e18, 5000);
        assertEq(count, 1000e18, "Should return taxTotalSupply when desiredOutput == surplus");
    }

    function test_desiredOutputExceedsSurplus() public pure {
        uint256 count = JBCashOuts.minCashOutCountFor(100 ether, 200 ether, 1000e18, 1000e18, 5000);
        assertEq(count, 1000e18, "Should return taxTotalSupply when desiredOutput > surplus");
    }

    function test_maxTaxRate_reverts() public {
        vm.expectRevert(JBCashOuts.JBCashOuts_DesiredOutputNotAchievable.selector);
        wrapper.minCashOutCountFor(100 ether, 50 ether, 1000e18, 1000e18, MAX_TAX);
    }
    ...
    function test_zeroTaxRate() public pure {
        ...
        uint256 count = JBCashOuts.minCashOutCountFor(100, 25, 200, 200, 0);
        assertEq(count, 50);
        assertGe(JBCashOuts.cashOutFrom(100, count, 200, 200, 0), 25);
    }
    ...
    function test_zeroTaxRate_roundsUp() public pure {
        ...
        uint256 count = JBCashOuts.minCashOutCountFor(7, 2, 100, 100, 0);
        assertGe(JBCashOuts.cashOutFrom(7, count, 100, 100, 0), 2);
    }

    function test_zeroSurplus() public pure {
        // surplus=0, desiredOutput=1 → desiredOutput >= surplus → return totalSupply.
        uint256 count = JBCashOuts.minCashOutCountFor(0, 1, 1000e18, 1000e18, 5000);
    ...
    function test_halfTax_knownValues() public pure {
        ...
        // Forward: cashOutFrom(100, 50, 100, 100, 5000) = 37.
        uint256 forwardResult = JBCashOuts.cashOutFrom(100, 50, 100, 100, 5000);
        assertEq(forwardResult, 37);

        // Inverse: to get 37 out, we need at least 50 tokens.
        uint256 count = JBCashOuts.minCashOutCountFor(100, 37, 100, 100, 5000);
        assertEq(count, 50);
        assertGe(JBCashOuts.cashOutFrom(100, count, 100, 100, 5000), 37);
    }

    function test_lowTax() public pure {
        // taxRate=1000 (10%).
        uint256 forwardResult = JBCashOuts.cashOutFrom(1000, 100, 1000, 1000, 1000);

        uint256 count = JBCashOuts.minCashOutCountFor(1000, forwardResult, 1000, 1000, 1000);
        assertGe(JBCashOuts.cashOutFrom(1000, count, 1000, 1000, 1000), forwardResult);
        assertLe(count, 100);
    }

    function test_highTax() public pure {
        // taxRate=9000 (90%).
        uint256 forwardResult = JBCashOuts.cashOutFrom(1000, 500, 1000, 1000, 9000);

        uint256 count = JBCashOuts.minCashOutCountFor(1000, forwardResult, 1000, 1000, 9000);
        assertGe(JBCashOuts.cashOutFrom(1000, count, 1000, 1000, 9000), forwardResult);
        assertLe(count, 500);
    }
    ...
    function test_realisticValues() public pure {
        ...
        uint256 forwardResult = JBCashOuts.cashOutFrom(surplus, cashOutAmount, totalSupply, totalSupply, taxRate);

        uint256 count = JBCashOuts.minCashOutCountFor(surplus, forwardResult, totalSupply, totalSupply, taxRate);
        assertGe(JBCashOuts.cashOutFrom(surplus, count, totalSupply, totalSupply, taxRate), forwardResult);
    ...
    function test_smallSurplus_largeTotalSupply() public pure {
        ...
        uint256 count = JBCashOuts.minCashOutCountFor(surplus, desiredOutput, totalSupply, totalSupply, taxRate);
        assertGe(JBCashOuts.cashOutFrom(surplus, count, totalSupply, totalSupply, taxRate), desiredOutput);
    }

    // =========================================================================
    // Round-trip: minCashOutCountFor(S, cashOutFrom(S, c, T, T, r), T, T, r) <= c
    // =========================================================================
        ...
        pure
    {
        ...
        uint256 output = JBCashOuts.cashOutFrom(surplus, cashOutCount, totalSupply, totalSupply, cashOutTaxRate);
        vm.assume(output > 0);

        uint256 recoveredCount = JBCashOuts.minCashOutCountFor(surplus, output, totalSupply, totalSupply, cashOutTaxRate);
    ...
    // =========================================================================
    // Correctness: cashOutFrom(S, minCashOutCountFor(S, out, T, T, r), T, T, r) >= out
    // =========================================================================
        ...
        pure
    {
        ...
        uint256 count = JBCashOuts.minCashOutCountFor(surplus, desiredOutput, totalSupply, totalSupply, cashOutTaxRate);

        uint256 actualOutput = JBCashOuts.cashOutFrom(surplus, count, totalSupply, totalSupply, cashOutTaxRate);
        ...
        pure
    {
        ...
        uint256 count1 = JBCashOuts.minCashOutCountFor(surplus, out1, totalSupply, totalSupply, cashOutTaxRate);
        uint256 count2 = JBCashOuts.minCashOutCountFor(surplus, out2, totalSupply, totalSupply, cashOutTaxRate);
        ...
        pure
    {
        ...
        uint256 lesserOutput = JBCashOuts.cashOutFrom(surplus, count - 1, totalSupply, totalSupply, cashOutTaxRate);
