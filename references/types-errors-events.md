...
- **`JBPrices` resolves a pair through one direct feed or one inverted feed — it never chains two.** For a pair it tries the registered feeds for `{pricingCurrency ← unitCurrency}`, then inverts the feeds registered for `{unitCurrency ← pricingCurrency}`, then repeats both against the project-0 defaults. So `{NATIVE ← uint32(usdc)}` has no route even when `{USD ← uint32(usdc)}` and `{USD ← NATIVE}` are both registered — the shared USD leg is not composed for you. Register a `JBRatioPriceFeed` (USDC/USD over ETH/USD) for the pair itself to supply that route. `ETH` (1) and `NATIVE_TOKEN` (61166) are distinct currency IDs and the matching feed between them is not chained either, so a composed feed must be registered for BOTH `{ETH ← uint32(usdc)}` and `{NATIVE ← uint32(usdc)}` to cover rulesets keyed on either.
