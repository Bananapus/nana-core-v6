...
- **ROOT + wildcard is unconditionally rejected.** `setPermissionsFor` reverts if `permissionIds` includes ROOT (1) and `projectId == 0` (wildcard). This is an intentional protocol-level invariant, not just an authorization check — even an account granting permissions to itself cannot create this combination. Rationale: ROOT grants all permissions (including future ones) for a project. Wildcard extends any permission to all projects. Their intersection would create a single key that can do anything to any project: drain funds, change rulesets, migrate terminals, mint tokens. No legitimate workflow requires this. The restriction eliminates an entire class of privilege-escalation attacks where a compromised operator key with ROOT+wildcard could cascade across the protocol. Note: ROOT for a *specific* project and non-ROOT permissions for wildcard are both permitted individually. Auditors should verify the exact boundary of the remaining authorization logic — particularly whether a ROOT operator on project X can escalate to ROOT on project Y through any indirect path.
