...
- **Fee-free payout flag prevents round-trip fee bypass.** When a project receives a fee-free intra-terminal payout, `_hasReceivedFeeFreePayout[projectId][token]` is permanently set to `true`. This disables the fee-free cashout exemption for that project/token pair — cash outs are always charged the 2.5% fee regardless of `cashOutTaxRate`. Without this, an attacker could route payouts through a pass-through project with `cashOutTaxRate=0` and cash out fee-free. Direct pay-in → cashout (no payout received) remains fee-free when `cashOutTaxRate=0`. Verified by tests in `TestFeeFreeCashOutBypass.sol`.
